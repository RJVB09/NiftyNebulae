...
namespace NiftyNebulae
{
    ...
    public class SkyFade : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (lineOfSightToSun)
            {
                sunCamAngle = Math.Acos(Vector3d.Dot(-sunRef.sunDirection, (Vector3d)tgt.transform.forward)) * (180.0 / Math.PI);
            ...
            totalFade = atmosFade + dayTimeFade + glareFade;  //Somehow goes NaN when the sun is focused
            //Main.Log("TotalFade: " + totalFade);
        }
